...
namespace DAL
{
   public class DAL_Previous
    {
        private long InvestmentPosting = 0L;

        string ConnString = DataAccess.ConnString;

        public void Insert_Record(BE_Previous NewBEObj)
        {
            SqlTransaction sqlTransaction = (SqlTransaction)null;
            try
            {
                string Qry = @"Insert into trans_PreviousData(RecordID,Date,Name,Amount,Description,Created_At,Modify_At,IsActive)
                                Values
                ...
                this.InvestmentPosting = int.Parse(DataAccess.GetMaxNO("RecordID", "Trans_PreviousData").ToString());
        ...
        public void Update_Record(BE_Previous NewBEObj)
        {
            SqlTransaction sqlTransaction = (SqlTransaction)null;
            try
            {
                string Qry = @"Update Trans_PreviousData
                                set
                                Date=@Date,
                                Name=@Name,
                                Amount=@Amount,
        ...
        public DataTable GetExpenseDetails(long RecordID)
        {
            ...
            {
                SqlConnection sqlConnection = new SqlConnection(this.ConnString);
                string Qry = @"Select * from Trans_PreviousData where RecordID='" + RecordID + "'";
